shopLib = (function () {
    ...
    let shopLib = {
        ...
        drawDefaultProductPanel: function () {
            ...
            const redirectFilterId = Number(
                sessionStorage.getItem("categoryFilterId")
            );
            lib.loadJsonByXhr(productInternal, function (productJson) {
                if (redirectFilterId && redirectFilterId !== -1) {
                    let filteredList = productJson.filter(
                        (product) =>
                            Number(product.categoryId) === redirectFilterId
                    );
        ...
        drawFilteredProductPanel: function (event) {
            ...
            if (
                location.pathname !== "/fend19-frontendproject-shop/index.php"
            ) {
            ...
            lib.loadJsonByXhr(productInternal, function (productJson) {
                if (allowedCategoryId === -1) {
                    ...
                    const newList = productJson.filter(
                        (product) => product.categoryId == allowedCategoryId
                    );
            ...
            productJson.forEach((item) => {
                const coverImage =
                    item.imageGallery.length > 0
                        ? "./img/product/" + item.imageGallery[0]
        ...
        searchProducts: function (event) {
            ...
            const keywordErrMsg = document.querySelector(
                ".invalidKeywordMessage"
            );
        ...
        hideSidePanel: function () {
            document
                .querySelector(".hamburger__bar-wrapper")
        ...
        getShoppingCart: function () {
            ...
            return !shoppingCart || Object.keys(shoppingCart).length === 0
                ? {}
                : shoppingCart;
        },
        ...
        drawLatestProductsPanel: function () {
            ...
            //cache selectors
            const latestProductsPanel = document.querySelector(
                "#latestProductsPanel"
            );
            let cardHtml = "";
            ...
            // get category json from Internal
            lib.loadJsonByXhr(latestProductsInternalUrl, function (
                latestProductsJson
            ) {
                // iterate over all categories
